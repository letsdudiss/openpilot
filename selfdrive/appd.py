...
def main():
  ...
  while 1:
    ...
    #Get EnableAppOverlay params which is set from Offroad UI toggles
    params = Params()
    sygic_enabled = params.get("EnableSygicOverlay", encoding='utf8') == "1"
    waze_enabled = params.get("EnableWazeOverlay", encoding='utf8') == "1"
    ...
    if not init_done:
      ...
      if is_onroad and not is_onroad_prev:
        for app in apps:
          ...
          if sygic_enabled and app.app == "com.sygic.aura":
            app.run()
          elif waze_enabled and app.app == "com.waze":
            app.run()
